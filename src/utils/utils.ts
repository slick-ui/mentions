 ...
 * @param positionOffset - position offset from the very beginning of text
 */
export const generateMentionPart = (
  mentionPartType: PartType,
  mention: MentionData,
  positionOffset = 0
): Part => {
...
export const defaultPlainStringGenerator = (
  { trigger }: PartType,
  { name }: MentionData
) => `${trigger}${name}`;
...
export const parseValue = (
  ...
  if (partTypes.length === 0) {
    ...
    const matches: RegexMatchResult[] = Array.from(
      (value || '').matchAll(regex) as unknown as RegexMatchResult[]
    );
 ...
 * @param suggestion
 */
export const getMentionValue = (trigger: string, suggestion: Mention) =>
  `${trigger}[${suggestion.name}](${suggestion.id})`;

export const defaultMentionTextStyle: StyleProp<TextStyle> = {
  fontWeight: 'bold',
  color: 'blue',
};
...
export const getMentionPartSuggestionKeywords = (
  ...
  partTypes.forEach(({ trigger, allowedSpacesCount = 1 }) => {
...
/**
 * get all mention values in value to some specified format
 *
 ...
 */
export const getAllMentionValues = (value: string) => {
  const data: MentionData[] = [];
  value.replace(
    mentionRegEx,
    (_: any, original: any, trigger: any, name: any, id: any) => {
      console.log('datas:', _, original, trigger, name, id);
      data.push({
        original,
        trigger,
        name,
